...
int main(void) {
    ...
    if (!(tvf = fopen("tv/pow_mod_p.txt", "r")))
    ...
    for (count = 0; count < 1000; count++) {
        ...
        if (read_u256(N + 3, tvf))
            error(1, "Couldn't read correct power");

        eim_pow_mod_p_u256(N + 2, N + 0, N + 1);
        if (eim_cmp_u256(N + 2, N + 3) != 0)
            error(2, "Incorrect power");
    }

    printf("[Mul by modulo p] Check %i test cases\n", count);
